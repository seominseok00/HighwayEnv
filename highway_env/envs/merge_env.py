...
class MergeEnv2(MergeEnv):
    ...
    def _make_vehicles(self) -> None:
        ...
        road = self.road
        ego_vehicle = self.action_type.vehicle_class(road,
                                                    road.network.get_lane(("j", "k", 0)).position(round(np.random.uniform(20, 25), 2), 0),
                                                    speed=round(np.random.uniform(5, 15), 2), target_speeds=[5, 15])
        
        road.vehicles.append(ego_vehicle)

        other_vehicles_type = utils.class_from_path(self.config["other_vehicles_type"])
        road.vehicles.append(other_vehicles_type(road, road.network.get_lane(("a", "b", 0)).position(round(np.random.uniform(55, 65), 2), 0), speed=round(np.random.uniform(8, 15), 2), target_speed=15))
        road.vehicles.append(other_vehicles_type(road, road.network.get_lane(("a", "b", 0)).position(round(np.random.uniform(20, 25), 2), 0), speed=round(np.random.uniform(8, 15), 2), target_speed=15))
        road.vehicles.append(other_vehicles_type(road, road.network.get_lane(("a", "b", 0)).position(round(np.random.uniform(0, 5), 2), 0), speed=round(np.random.uniform(8, 15), 2), target_speed=15))
